...
> 1. `:src` 圖片綁定：畫面上正確顯示出 `imgUrl` 指定的圖片（沒有出現破圖圖示）。把 `imgUrl` 的值改成另一個網址後，存檔回瀏覽器，圖片立刻換成新的那張，不用手動重新整理頁面。
> 2. `:class` 物件語法：點擊「切換樣式」按鈕後，文字立刻變成紅色且加粗；再點一次按鈕，文字變回原本的黑色、非粗體樣式。
> 3. `:style` 物件語法：點擊「切換背景色」按鈕後，色塊背景從藍色變成綠色；再點一次按鈕，背景變回藍色。
